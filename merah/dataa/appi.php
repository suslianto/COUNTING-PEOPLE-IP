...
function coun($page) {
        ...
        foreach ($data["data"] as $val) {
              ...
              curl_setopt_array($ceh, array(
                ...
                CURLOPT_RETURNTRANSFER => 1,
                CURLOPT_TIMEOUT => 60,
                CURLOPT_URL => "https://192.168.1.50:8098/api/person/get/".$pin."?access_token=416012CD899F5A14E96E09F08AF0D264",
                CURLOPT_HTTPHEADER => $head,
                CURLOPT_VERBOSE => true,
              ));
              $geet = curl_exec($ceh);
        ...
        foreach ($tes as $val) {
            if ($val != "") {
              ...
              foreach ($inkom as $kom) {
                if(in_array($kom, $outkom)) {
                  continue;
                } else {
                  $curperson["data"][] = array(
                      "name" => $kom["name"],
                      "foto" => $kom["foto"],
                  );
                }
  ...
  foreach ($db as $key) {
      ...
      foreach ($result["data"] as $val) {
        if ($key == $val["dept"]) {
          ...
          foreach ($val["person"]["data"] as $pecah ) {
            $perso["data"][] = array(
              "name" => $pecah["name"],
              "foto" => $pecah["foto"],
            );
